  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (initialRx) {
        ...
        toast.success(t('prescriptionForm.toasts.updated'));
      } else {
        ...
        toast.success(t('prescriptionForm.toasts.added'));
      }
      
      setIsOpen(false);
      onCreated();
    } catch (e: any) {
      toast.error(t('prescriptionForm.toasts.addFailed', { reason: e.message }));
    } finally {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      {!initialRx && (
        <DialogTrigger asChild>
          <button className="flex items-center gap-2 bg-[color:var(--color-carely-primary)] text-white px-4 py-2 rounded-full font-jakarta font-medium shadow-[0_2px_10px_var(--color-carely-tertiary)] hover:opacity-90">
            <Plus className="w-4 h-4" /> {t('prescriptionForm.triggerButton')}
          </button>
        </DialogTrigger>
      )}
      <DialogContent className="sm:max-w-[425px] bg-[color:var(--color-carely-surface-lowest)] border-[color:var(--color-carely-surface-high)] rounded-2xl">
        <DialogHeader>
          <DialogTitle className="font-lexend text-[color:var(--color-carely-on-surface)] text-xl">{initialRx ? t('prescriptionForm.editTitle') : t('prescriptionForm.newTitle')}</DialogTitle>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="flex flex-col gap-4 mt-2">
           <input required type="text" placeholder={t('prescriptionForm.placeholders.medicineName')} value={name} onChange={e => setName(e.target.value)} className="w-full bg-[color:var(--color-carely-surface-low)] border-none rounded-xl p-3 font-jakarta text-[color:var(--color-carely-on-surface)]" />
           
           <div className="grid grid-cols-2 gap-4">
             <div>
               <label className="text-[10px] uppercase text-[color:var(--color-carely-on-surface-variant)] block mb-1 font-semibold pl-1">{t('prescriptionForm.labels.startDate')}</label>
             ...
             <div>
               <label className="text-[10px] uppercase text-[color:var(--color-carely-on-surface-variant)] block mb-1 font-semibold pl-1">{t('prescriptionForm.labels.endDate')}</label>
           ...
           <div className="flex items-center gap-2">
             ...
             <label htmlFor="customToggle" className="text-sm font-jakarta text-[color:var(--color-carely-on-surface)] select-none cursor-pointer">{t('prescriptionForm.labels.useCustom')}</label>
           </div>

           {useCustom ? (
             <textarea required placeholder={t('prescriptionForm.placeholders.customSchedule')} value={notes} onChange={e => setNotes(e.target.value)} className="w-full bg-[color:var(--color-carely-surface-low)] border-none rounded-xl p-3 font-jakarta text-[color:var(--color-carely-on-surface)] resize-none h-20" />
           ) : (
             <div className="grid grid-cols-4 gap-2">
               <div><label className="text-[10px] uppercase text-[color:var(--color-carely-on-surface-variant)] block text-center mb-1 font-semibold">{t('prescriptionForm.labels.morning')}</label>
               ...
               <div><label className="text-[10px] uppercase text-[color:var(--color-carely-on-surface-variant)] block text-center mb-1 font-semibold">{t('prescriptionForm.labels.afternoon')}</label>
               ...
               <div><label className="text-[10px] uppercase text-[color:var(--color-carely-on-surface-variant)] block text-center mb-1 font-semibold">{t('prescriptionForm.labels.evening')}</label>
               ...
               <div><label className="text-[10px] uppercase text-[color:var(--color-carely-on-surface-variant)] block text-center mb-1 font-semibold">{t('prescriptionForm.labels.night')}</label>
               ...
               <div className="col-span-4 mt-2">
                  <input type="text" placeholder={t('prescriptionForm.placeholders.notes')} value={notes} onChange={e => setNotes(e.target.value)} className="w-full bg-[color:var(--color-carely-surface-low)] border-none rounded-xl p-3 font-jakarta text-[color:var(--color-carely-on-surface)]" />
               </div>
             </div>
           )}
          ...
          <button disabled={isSubmitting} type="submit" className="mt-4 w-full bg-[color:var(--color-carely-primary)] text-[color:var(--color-carely-on-primary)] font-jakarta font-semibold text-lg py-3 rounded-xl disabled:opacity-50">
            {isSubmitting ? t('prescriptionForm.saving') : t('prescriptionForm.save')}
          </button>
        </form>
      </DialogContent>
    </Dialog>
